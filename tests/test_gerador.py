...
class TestGeradorEgera:
    ...
    def test_calcular_maximo_palavras(self, gerador):
        ...
        esperado = (
            len(gerador.silabas_tonicas) * len(gerador.silabas_medias) * len(gerador.silabas_finais)
        )
        assert maximo == esperado, f"Esperado {esperado}, mas obteve {maximo}"
...
class TestIntegracao:
    ...
    def test_fluxo_completo_geracao_validacao(self, gerador):
        ...
        for palavra in palavras:
            ...
            assert gerador.validar_proparoxitona(palavra), f"Palavra {palavra} falhou na validação"
    ...
    def test_arquivo_palavras_completas_existe_e_contem_todas(self, gerador):
        ...
        if os.path.exists("palavras_completas.txt"):
            with open("palavras_completas.txt", encoding="utf-8") as f:
